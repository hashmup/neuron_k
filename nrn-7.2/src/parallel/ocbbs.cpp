...
#ifdef KPLUS_GATHER_SCATTER
	void nrnmpi_dbl_gather(double*, double*, int, int);
	void nrnmpi_int_gather(int*, int*, int, int);
	void nrnmpi_dbl_scatter(double*, double*, int, int);
	void nrnmpi_int_scatter(int*, int*, int, int);
#endif /* KPLUS_GATHER_SCATTER */
#ifdef KPLUS_SPAWN
	extern void nrnmpi_set_parent_comm();
	extern void nrnmpi_set_world_comm();
#endif /* KPLUS_SPAWN */
...
#ifdef KPLUS_GATHER_SCATTER
static double gather_d(void*) {
	Vect* svec = vector_arg(1);
	Vect* dvec = vector_arg(2);
	int sendcount = *getarg(3);
	int root = *getarg(4);
	vector_resize(dvec, nrnmpi_numprocs * sendcount);
	double* spx = vector_vec(svec);
	double* dpx = vector_vec(dvec);

#if NRNMPI
	if (nrnmpi_numprocs > 1) {
		nrnmpi_dbl_gather(spx, dpx, sendcount, root);
		errno = 0;
	}else{
		dpx = spx;
	}
...
#if NRNMPI
	if (nrnmpi_numprocs > 1) {
		spx_i = (int*)malloc( sendcount * sizeof(int) );
		dpx_i = (int*)malloc( nrnmpi_numprocs * sendcount * sizeof(int) );
		for(i = 0; i < sendcount; i++) { spx_i[i] = (int)spx[i]; }

		nrnmpi_int_gather(spx_i, dpx_i, sendcount, root);
		errno = 0;

		for(i = 0; i < sendcount*nrnmpi_numprocs; i++) { dpx[i] = (double)dpx_i[i]; }
		free(spx_i);
		free(dpx_i);
	}else{
		dpx = spx;
...
static double scatter_d(void*) {
	Vect* svec = vector_arg(1);
	Vect* dvec = vector_arg(2);
	int sendcount = *getarg(3);
	int root = *getarg(4);
	vector_resize(dvec, sendcount);
	double* spx = vector_vec(svec);
	double* dpx = vector_vec(dvec);

#if NRNMPI
	if (nrnmpi_numprocs > 1) {
		nrnmpi_dbl_scatter(spx, dpx, sendcount, root);
		errno = 0;
	}else{
		dpx = spx;
	}
...
#if NRNMPI
	if (nrnmpi_numprocs > 1) {
		spx_i = (int*)malloc( nrnmpi_numprocs * sendcount * sizeof(int) );
		dpx_i = (int*)malloc( sendcount * sizeof(int) );
		for(i = 0; i < sendcount*nrnmpi_numprocs; i++) { spx_i[i] = (int)spx[i]; }

		nrnmpi_int_scatter(spx_i, dpx_i, sendcount, root);
		errno = 0;

		for(i = 0; i < sendcount; i++) { dpx[i] = (double)dpx_i[i]; }
		free(spx_i);
		free(dpx_i);
	}else{
		dpx = spx;
...
#ifdef KPLUS_SPAWN
#if NRNMPI
static double set_parent_comm(void* v ) {
	nrnmpi_set_parent_comm();
	return 0.;
}

static double set_world_comm(void* v) {
	nrnmpi_set_world_comm();
	return 0.;
}
...
#ifdef KPLUS_SPAWN
	"set_parent_comm", set_parent_comm,
	"set_world_comm", set_world_comm,
#endif /* KPLUS_GATHER_SCATTER */
