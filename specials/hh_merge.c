...
extern Memb_func* memb_func;
 ...
 "setdata_hh_merge", _hoc_setdata,
 "rates_hh_merge", _hoc_rates,
 "vtrap_hh_merge", _hoc_vtrap,
 0, 0
};
#define vtrap vtrap_hh_merge
...
#define _gth 0
#define htau_hh_merge _thread1data[0]
#define htau _thread[_gth]._pval[0]
#define hinf_hh_merge _thread1data[1]
#define hinf _thread[_gth]._pval[1]
#define mtau_hh_merge _thread1data[2]
#define mtau _thread[_gth]._pval[2]
#define minf_hh_merge _thread1data[3]
#define minf _thread[_gth]._pval[3]
#define ntau_hh_merge _thread1data[4]
#define ntau _thread[_gth]._pval[4]
#define ninf_hh_merge _thread1data[5]
#define ninf _thread[_gth]._pval[5]
#define usetable usetable_hh_merge
 double usetable = 1;
 /* some parameters have upper and lower limits */
 static HocParmLimits _hoc_parm_limits[] = {
 "gl_hh_merge", 0, 1e+09,
 "gkbar_hh_merge", 0, 1e+09,
 "gnabar_hh_merge", 0, 1e+09,
 "usetable_hh_merge", 0, 1,
 0,0,0
};
 ...
 "htau_hh_merge", "ms",
 "ntau_hh_merge", "ms",
 "gnabar_hh_merge", "S/cm2",
 "gkbar_hh_merge", "S/cm2",
 "gl_hh_merge", "S/cm2",
 "el_hh_merge", "mV",
 "gna_hh_merge", "S/cm2",
 "gk_hh_merge", "S/cm2",
 ...
 "minf_hh_merge", &minf_hh_merge,
 "hinf_hh_merge", &hinf_hh_merge,
 "ninf_hh_merge", &ninf_hh_merge,
 "mtau_hh_merge", &mtau_hh_merge,
 "htau_hh_merge", &htau_hh_merge,
 "ntau_hh_merge", &ntau_hh_merge,
 "usetable_hh_merge", &usetable_hh_merge,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 7);
 	...
 	ivoc_help("help ?1 hh_merge /volume1/home/hp120263/k00634/neuron_kplus/specials/sparc64/hh_merge.mod\n");
...
#pragma loop norecurrence
  for (_iml = 0; _iml < _cntml; _iml++){
    FLOAT n_inf, m_inf, h_inf, _gna, _gk, _n, _m, _h;
    const int v_i = _i_table[_iml];
    ...
    */
    _n = n_table[_iml] + (1.0f - EXP(-local_dt/TABLE_N_TAU(v_i))) * (n_inf + _theta_table[_iml] * (TABLE_N_INF(v_i+1) - n_inf) - n_table[_iml]);
    _m = m_table[_iml] + (1.0f - EXP(-local_dt/TABLE_M_TAU(v_i))) * (m_inf + _theta_table[_iml] * (TABLE_M_INF(v_i+1) - m_inf) - m_table[_iml]);
    _h = h_table[_iml] + (1.0f - EXP(-local_dt/TABLE_H_TAU(v_i))) * (h_inf + _theta_table[_iml] * (TABLE_H_INF(v_i+1) - h_inf) - h_table[_iml]);

    _gna = gnabar_table[_iml] * _m * _m * _m * _h;
    _gk = gkbar_table[_iml] * _n * _n * _n * _n;
    g_table[_iml] = _gna + _gk + gl_table[_iml];
    il_table[_iml] = gl_table[_iml] * (v_table[_iml] - el_table[_iml]) 
      + _gna * (v_table[_iml] - ena_table[_iml])
      + _gk * (v_table[_iml] - ek_table[_iml]);
    n_table[_iml] = _n;
    m_table[_iml] = _m;
    h_table[_iml] = _h;
  }
